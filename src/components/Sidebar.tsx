...
const Sidebar: React.FC<SidebarProps> = ({ isCollapsed }) => {
    ...
    const iconClass = "text-2xl"; // Set a consistent font size for all icons
    const Explore = [
        { id: 1, name: "Trending", icon: <AiFillFire className={iconClass} /> },
        { id: 2, name: "Shopping", icon: <AiOutlineShopping className={iconClass} /> },
        { id: 3, name: "Music", icon: <IoMusicalNoteOutline className={iconClass} /> },
        { id: 4, name: "Movies", icon: <PiFilmSlate className={iconClass} /> },
        { id: 5, name: "Gaming", icon: <IoGameControllerOutline className={iconClass} /> },
        { id: 6, name: "News", icon: <HiOutlineNewspaper className={iconClass} /> },
        { id: 7, name: "Podcasts", icon: <MdOutlinePodcasts className={iconClass} /> },
    ];
            ...
            <div className="px-9 h-full">
                <div className="flex flex-col items-center space-y-10">
                    ...
                    <div className="hover:bg-gray-200 p-2 rounded-full">
                        <SiYoutubeshorts className={`cursor-pointer ${iconClass}`} />
                    </div>
                    <div className="hover:bg-gray-200 p-2 rounded-full">
                        <MdOutlineSubscriptions className={`cursor-pointer ${iconClass}`} />
                    </div>
                    <div className="hover:bg-gray-200 p-2 rounded-full">
        ...
        <div className="relative h-[calc(100vh-6.625rem)] mt-24 w-[18%]">
            <div className="px-6 h-full overflow-y-hidden hover:overflow-y-scroll">
                <div className="space-y-2 hidden md:block">
                    ...
                    <div className="flex items-center space-x-4 hover:bg-gray-200 rounded-xl p-2">
                        <RiPlayList2Fill className={`cursor-pointer ${iconClass}`} />
                        <span className="cursor-pointer">Playlists</span>
                    </div>
                    <div className="flex items-center space-x-4 hover:bg-gray-200 rounded-xl p-2">
                        <MdOutlineWatchLater className={`cursor-pointer ${iconClass}`} />
                        <span className="cursor-pointer">Watch Later</span>
                    </div>
                    <div className="flex items-center space-x-4 hover:bg-gray-200 rounded-xl p-2">
                        <AiOutlineLike className={`cursor-pointer ${iconClass}`} />
                        <span className="cursor-pointer">Likes</span>
                    </div>
                    <div className="flex items-center space-x-4 hover:bg-gray-200 rounded-xl p-2">
                        <RiVideoLine className={`cursor-pointer ${iconClass}`} />
                    ...
                    <div className="space-y-2">
                        {Explore.map((item) => (
                            ...
                            >
                                <div className={`cursor-pointer ${iconClass}`}>{item.icon}</div>
